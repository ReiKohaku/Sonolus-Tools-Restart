    ...
    html(): string {
      return converter.makeHtml(this.markdown)
        ...
        .replace(/<img(.*?)src="(.*?)"(.*?)>/g, (line: string) => {
          ...
          if (matches)
            return `<img${matches[1]}src="/github/raw/ReiKohaku/Sonolus-Tools-Data/main${matches[2].startsWith('/') ? '' : '/'}${matches[2]}"${matches[3]}>`;
          return line;
        });
    }
  },
    ...
    redirectImages() {
      ...
      if (el) {
        ...
        blocks.forEach((block) => {
          ...
          if (matches)
            block.src = ('/github/raw/ReiKohaku/Sonolus-Tools-Data/main' + (matches[2].startsWith('/') ? '' : '/') + matches[2]) || block.src;
        });
      }
    },
    highlightHtml() {
